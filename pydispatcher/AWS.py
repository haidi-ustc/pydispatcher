...
from pydispatcher.Batch import Batch 
...
class AWS(Batch):
    ...
    @staticmethod
    def map_aws_status_to_pydisp_status(aws_status):
            ...
            for status in ['SUBMITTED', 'PENDING', 'RUNNABLE', 'STARTING', 'RUNNING','SUCCEEDED', 'FAILED']:
                ...
                while nextToken is not None:
                    ...
                    for job_dict in status_list:
                        cls._job_id_map_status.update({job_dict['jobId']: cls.map_aws_status_to_pydisp_status(job_dict['status'])})
    ...
    def job_id(self, values):
        ...
        self.__class__._job_id_map_status[self._job_id] = self.map_aws_status_to_pydisp_status(response.get('status', 'SUBMITTED'))
